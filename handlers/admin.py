...
async def cb_settings(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    await callback.message.edit_text(
        ...
        f"💰 Цена 1 спонсора: <b>{r_per_sp.value if r_per_sp and r_per_sp.value else '0'}</b> (макс. 5⭐ итого)\n"
...
async def cb_set_reward_per_sponsor(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await _ask_setting(callback, state, AdminSettingsStates.reward_per_sponsor,
                       "💰 Введи награду за 1 спонсора (число, напр. 1.5).\nИтого = цена × кол-во спонсоров, макс. 5⭐:")
