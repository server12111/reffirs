_MAP = {
    "⭐️": "5438496463044752972",
    "⭐":  "5438496463044752972",
    "👤":  "5373012449597335010",
    "📋":  "5435970940670320222",
    "🎟":  "5431653802753159903",
    "🔗":  "5271604874419647061",
    "➕":  "5397916757333654639",
    "🔥":  "5389038097860144794",
    "🗓":  "5413879192267805083",
    "🎁":  "5427225953463972959",
    "👥":  "5325533093673588447",
    "🏆":  "5280769763398671636",
    "🎮":  "5350708744558753862",
    "➡️": "5346105514575025401",
    "⏭️": "5321335209818339164",
    "⏭":  "5321335209818339164",
    "📢":  "5370599459661045441",
    "⚽️": "5375159220280762629",
    "⚽":  "5375159220280762629",
    "🏀":  "5384088040677319401",
    "🎳":  "5370853837689070338",
    "🎲":  "5384474763827620477",
    "🎰":  "5915833712368424979",
    "⚔️": "5453991094435997597",
    "⚔":  "5453991094435997597",
    "📄":  "5370604433233177619",
    "📜":  "5370604433233177619",
    "🔁":  "5361993818373655559",
    "📈":  "5269460053651366623",
    "📉":  "5271811599785534382",
    "📆":  "5431897022456145283",
    "💸":  "5391218287684116768",
    "💰":  "5309795500277403547",
    "🔍":  "5258274739041883702",
    "✅":  "5462919317832082236",
    "❌":  "5210952531676504517",
    "◀️": "5318991467639756533",
    "⏳":  "5175181110572745347",
    "🛡":  "5017108172138087141",
    "⚠️": "5447644880824181073",
    "👋":  "5472055112702629499",
    "💫":  "5469744063815102906",
    "📌":  "5462912132351797094",
    "🏠":  "5416041192905265756",
    "🚀":  "5366230850855777158",
}


import re as _re

# Sorted longest-first so e.g. "⭐️" matches before "⭐"
_SORTED = sorted(_MAP.keys(), key=len, reverse=True)
_PATTERN = _re.compile('(' + '|'.join(_re.escape(e) for e in _SORTED) + ')')


def pe(text: str) -> str:
    """Replace known emoji with premium tg-emoji HTML tags (single pass)."""
    return _PATTERN.sub(lambda m: f'<tg-emoji emoji-id="{_MAP[m.group(0)]}">{m.group(0)}</tg-emoji>', text)


def strip_pe(text: str) -> str:
    """Remove <tg-emoji> tags, keeping the fallback plain emoji."""
    return _re.sub(r'<tg-emoji[^>]*>([^<]*)</tg-emoji>', r'\1', text, flags=_re.IGNORECASE)
